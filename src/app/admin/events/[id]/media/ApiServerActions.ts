...
export async function fetchAgendaFlyerServer(eventId: string | number): Promise<EventMediaDTO | null> {
  const params = new URLSearchParams({
  ...
  const res = await fetchWithJwtRetry(`${getApiBase()}/api/event-medias?${params.toString()}`, {
    cache: 'no-store',
  });
  if (!res.ok) return null;
  const data = await res.json();
